...
class ProbabilityEngine:
    ...
    def _apply_sunk_ships(self, sunk_ships, misses_mask):
        ...
        for spec in self.config.ships:
            available_by_length[spec.length] = available_by_length.get(spec.length, 0) + spec.count
        ...
        for sunk_ship in sunk_ships:
            ...
            if available_by_length.get(sunk_ship.length, 0) <= 0:
        ...
        remaining_lengths = []
        for length, count in available_by_length.items():
            remaining_lengths.extend([length] * count)
        remaining_lengths.sort(reverse=True)
    ...
    def evaluate(self, cell_states, sunk_ships=None) -> EvaluationResult:
        ...
        required_mask = hits_mask & ~fixed_mask
